...
#-------------------------------
	print "Now we are going send temp data to the server"
	...
	try:
		#send data
		print >>sys.stderr, 'Sending temp data to server "%s"' % targetT
			
		print "*--------------------" 
		targetTstr=repr(targetT)
		targetTstr=targetTstr[:4]
		print targetTstr		
		sock.sendall(' Temperature is : %s ' % targetTstr)
		print "!--------------------"
		
	finally:
	        print >>sys.stderr, 'closing socket tempereature\n\n\n'
	        sock.close()
...
#-------------------------------
	print "Now we are going send humidity data to the server"
	...
	try:
		...
		print >>sys.stderr, 'Sending humidity data  to server "%s"' % rh
		#print "this is targetT %s" % rh
		
		print "*--------------------" 
		humidityStr=repr(rh)	
		humidityStr=humidityStr[:4]	
		print humidityStr
		sock.sendall(' Humidity is : %s ' % humidityStr)
		print "!--------------------"
		
	finally:
	        print >>sys.stderr, 'closing socket humidity\n\n\n'
	        sock.close()
